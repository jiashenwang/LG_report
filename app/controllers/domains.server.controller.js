 ...
 * List of Domains
 */
exports.list = function(req, res) { Domain.find().sort('-created').populate('user', 'displayName').exec(function(err, domains) {
		if (err) {
			return res.status(400).send({
				message: errorHandler.getErrorMessage(err)
			});
 ...
 * Domain middleware
 */
exports.domainByID = function(req, res, next, id) { Domain.findById(id).populate('user', 'displayName').exec(function(err, domain) {
